...
class PrivadoInstalacionHorarioEspecialControllerTest {
	...
	@InjectMocks
	private PrivadoInstalacionHorarioEspecialController controller;
	...
	void searchTest() throws Exception {
		...
		when(instalacionHorarioEspecialService.getPageByFilter(any(), any())).thenReturn(new PageImpl<>(List.of()));
	...
	void eliminarExitosoTest() throws Exception {
		...
		Long id = 10L;
		when(instalacionHorarioEspecialService.canWrite(id)).thenReturn(true);

		mockMvc.perform(get("/privado/instalacion/{idInstalacion}/horario/especial/{id}/eliminar", idInstalacion, id))
				.andExpect(status().is3xxRedirection())
				.andExpect(redirectedUrl("/privado/instalacion/1/horario/especial"));

		verify(instalacionHorarioEspecialService).eliminar(id);
	}
